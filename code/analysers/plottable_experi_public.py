...
output_dir = os.path.join(PROJECT_ROOT, "analysis", "plottable_imgs_experimental")
os.makedirs(output_dir, exist_ok=True)

def plottable_experi_public():
    race_folders = sorted(
    ...
    COLUMNS = ["Overall Rank", "Team", "Name", "Total Points"]
    ...
    def build_column_defs(df: pd.DataFrame) -> list:
        ...
        return [
            ColumnDefinition(
                name="Overall Rank",
                width=1.8,
                textprops={"ha": "center", "fontsize": 9, "fontweight": "bold"},
            ),
            ColumnDefinition(
                name="Team",
                width=1.8,
    ...
    for folder in race_folders:
        ...
        # ── Save ───────────────────────────────────────────────────────────────────
        out_path = os.path.join(output_dir, f"race_{race_num}_playerinfotest.png")
